...
def guessMissingRatings(addtoinput=False):
    calculatedratings = {}
    for user in userlist:
        i = userlist.index(user)
        ratings = critics[user]
        for movie in movielist:
            if movie not in ratings:
                j = movielist.index(movie)
                rating = np.dot(params[i],np.concatenate((np.array([1.],dtype=float), coords[j])))
                if addtoinput:
                    ratings[movie] = rating
                if user not in calculatedratings:
                    calculatedratings[user]={}
                calculatedratings[user][movie] = rating
...
#        xprev = xk
    xopt=fmin_bfgs(costfunction,x,fprime=gradientfunction,args=(critics, dimension,userlist,movielist,lamda), maxiter=300)
    log.debug('xopt is \n' +  repr(xopt))
    nu = len(userlist)
    nm = len(movielist)
    ...
    verify()
    missingratings = guessMissingRatings()
    print 'missing ratings are :'
    print missingratings
 #   print 'fopt ', fopt
 #   print 'gopt ',gopt
 #   print 'Bopt ' , Bopt
